	...
	<div class="container">
		...
		<div class="row">
			<div id="blog" class="col-lg-8">
				...
				<br>
				<c:choose>
					<c:when test="${cateList != null && fn:length(cateList)>0 }">
						<c:forEach items="${cateList }" var="clist">

							<h3>
								<a
									href="<%=basePath%>comment.html?action=readMe&artId=${clist.id }"
									target="_blank">${clist.title }</a>
							</h3>
							<p>
								<i class="glyphicon glyphicon-user"></i> 
								 ${clist.name } | <i class="glyphicon glyphicon-calendar"></i>
								 ${clist.publishTime } |
								阅读 ${clist.count } 次
							</p>
							...
							<a class="btn btn-primary"
								href="<%=basePath%>comment.html?action=readMe&artId=${clist.id }">Read
