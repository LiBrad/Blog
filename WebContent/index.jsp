	...
	<div class="container">
		...
		<div class="row">
			<div id="blog" class="col-lg-8">
				...
				<br>
				<c:choose>
					<c:when test="${pageList != null && fn:length(pageList)>0 }">
						<c:forEach items="${pageList }" var="plist">

							<h3>
								<a
									href="<%=basePath%>comment.html?action=readMe&artId=${plist.id }"
									target="_blank">${plist.title }</a>
							</h3>
							<p>
								<i class="glyphicon glyphicon-user"></i> 
								 ${plist.name } | <i class="glyphicon glyphicon-calendar"></i>
								 ${plist.publishTime } |
								阅读 ${plist.count } 次
							</p>
							...
							<a class="btn btn-primary"
								href="<%=basePath%>comment.html?action=readMe&artId=${plist.id }">Read
